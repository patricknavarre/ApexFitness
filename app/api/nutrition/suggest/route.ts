...
const SYSTEM_PROMPT = `You are a nutrition expert. Given the user's remaining daily calories and macros (protein, carbs, fat in grams), suggest ONE simple meal or snack that fits within these numbers.
  ...
  try {
    ...
    const { text } = await generateText({
      ...
      messages: [
        {
          role: 'user',
          content: `Remaining today: ${cal} calories, ${p}g protein, ${c}g carbs, ${f}g fat. Suggest one meal or snack that fits.`,
        },
      ],
      maxTokens: 300,
    });
